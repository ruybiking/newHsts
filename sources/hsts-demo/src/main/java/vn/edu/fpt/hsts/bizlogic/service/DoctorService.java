...
//                                LOGGER.info("Create new medicine", medicine.getName());
    ...
    public PracticeDataModel getPracticeDataOfPatient(final Appointment appointment) {
        LOGGER.info(IConsts.BEGIN_METHOD);
        try {
            LOGGER.info("appointment[{}]", appointment);
            PracticeResultModel resultModel = new PracticeResultModel();
            ...
            if (null != oldAppointment) {
                PracticeDataModel pdm = new PracticeDataModel();
                Treatment treatment = treatmentRepo.findLastTreatmenByAppointmentId(oldAppointment.getId()).get(0);
                //Code cua Quy
                List<String> listTime = new ArrayList<String>();
                List<String> listCalories = new ArrayList<String>();
                List<String> listDistance = new ArrayList<String>();
                pdm.setKcalEstimate(treatment.getCaloriesBurnEveryday() + "");
                List<MedicalRecordData> medicalRecordDataList = medicalRecordDataRepo.findRecordDataByAppointmentId(oldAppointment.getId());
                for(int i = 0; i < medicalRecordDataList.size(); i++) {
                    MedicalRecordData mrd = medicalRecordDataList.get(i);
                    SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
                    listTime.add(sdf.format(mrd.getCollectedDate()));
                    List<PropertyRecord> propertyRecordList = propertyRecordRepo.findAllPropertyRecordByMrd(mrd.getId());
                    for(int j = 0; j < propertyRecordList.size(); j++) {
                        if(propertyRecordList.get(j).getParamMeasurement().getId() == 2) {
                            listCalories.add(propertyRecordList.get(j).getParamMeasurementValue());
                        }
                        if(propertyRecordList.get(j).getParamMeasurement().getId() == 3) {
                            listDistance.add(propertyRecordList.get(j).getParamMeasurementValue());
                        }
                    }
                }
                pdm.setListPatientTime(listTime);
                pdm.setListPatientDistance(listDistance);
                pdm.setListPatientCalories(listCalories);
                return pdm;
                //Code cua Quy
            }
            return null;
